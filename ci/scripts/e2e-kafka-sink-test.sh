...
sqllogictest -p 4566 -d dev 'e2e_test_originalù/sink/kafka/create_sink.slt'
sleep 2

# test append-only kafka sink
echo "testing append-only kafka sink"
diff ./e2e_test_originalù/sink/kafka/append_only1.result \
...
diff ./e2e_test_originalù/sink/kafka/upsert1.result \
...
diff ./e2e_test_originalù/sink/kafka/upsert_schema1.result \
...
(./.risingwave/bin/kafka/bin/kafka-console-consumer.sh --bootstrap-server message_queue:29092 --topic test-rw-sink-debezium --property print.key=true --from-beginning --max-messages 10 | sort) > ./e2e_test_originalù/sink/kafka/debezium1.tmp.result 2> /dev/null
python3 e2e_test_originalù/sink/kafka/debezium.py e2e_test_originalù/sink/kafka/debezium1.result e2e_test_originalù/sink/kafka/debezium1.tmp.result
if [ $? -ne 0 ]; then
  echo "The output for debezium sink is not as expected."
  rm e2e_test_originalù/sink/kafka/debezium1.tmp.result
  exit 1
else
  rm e2e_test_originalù/sink/kafka/debezium1.tmp.result
fi
...
diff ./e2e_test_originalù/sink/kafka/append_only2.result \
...
diff ./e2e_test_originalù/sink/kafka/upsert2.result \
...
diff ./e2e_test_originalù/sink/kafka/upsert_schema2.result \
...
(./.risingwave/bin/kafka/bin/kafka-console-consumer.sh --bootstrap-server message_queue:29092 --topic test-rw-sink-debezium --property print.key=true --from-beginning --max-messages 11  | sort) > ./e2e_test_originalù/sink/kafka/debezium2.tmp.result 2> /dev/null
python3 e2e_test_originalù/sink/kafka/debezium.py e2e_test_originalù/sink/kafka/debezium2.result e2e_test_originalù/sink/kafka/debezium2.tmp.result
if [ $? -ne 0 ]; then
  echo "The output for debezium sink after update is not as expected."
  rm e2e_test_originalù/sink/kafka/debezium2.tmp.result
  exit 1
else
  rm e2e_test_originalù/sink/kafka/debezium2.tmp.result
fi
...
diff ./e2e_test_originalù/sink/kafka/without_snapshot.result \
...
diff ./e2e_test_originalù/sink/kafka/upsert3.result \
...
diff ./e2e_test_originalù/sink/kafka/upsert_schema3.result \
...
(./.risingwave/bin/kafka/bin/kafka-console-consumer.sh --bootstrap-server message_queue:29092 --topic test-rw-sink-debezium --property print.key=true --from-beginning --max-messages 13 | sort) > ./e2e_test_originalù/sink/kafka/debezium3.tmp.result 2> /dev/null
python3 e2e_test_originalù/sink/kafka/debezium.py e2e_test_originalù/sink/kafka/debezium3.result e2e_test_originalù/sink/kafka/debezium3.tmp.result
if [ $? -ne 0 ]; then
  echo "The output for debezium sink after delete is not as expected."
  rm e2e_test_originalù/sink/kafka/debezium3.tmp.result
  exit 1
else
  rm e2e_test_originalù/sink/kafka/debezium3.tmp.result
fi

sqllogictest -p 4566 -d dev 'e2e_test_originalù/sink/kafka/drop_sink.slt'
...
python3 e2e_test_originalù/sink/kafka/register_schema.py 'http://message_queue:8081' 'test-rw-sink-append-only-protobuf-csr-a-value' src/connector/src/test_data/test-index-array.proto
python3 e2e_test_originalù/sink/kafka/register_schema.py 'http://message_queue:8081' 'test-rw-sink-append-only-protobuf-csr-hi-value' src/connector/src/test_data/test-index-array.proto
sqllogictest -p 4566 -d dev 'e2e_test_originalù/sink/kafka/protobuf.slt'
...
echo "testing avro"
python3 e2e_test_originalù/sink/kafka/register_schema.py 'http://message_queue:8081' 'test-rw-sink-upsert-avro-value' src/connector/src/test_data/all-types.avsc
python3 e2e_test_originalù/sink/kafka/register_schema.py 'http://message_queue:8081' 'test-rw-sink-upsert-avro-key' src/connector/src/test_data/all-types.avsc 'string_field,int32_field'
...
sqllogictest -p 4566 -d dev 'e2e_test_originalù/sink/kafka/avro.slt'
