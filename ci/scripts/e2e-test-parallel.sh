...
sqllogictest "${host_args[@]}" -d dev './e2e_test_originalù/streaming/**/*.slt' -j 16 --junit "parallel-streaming-${profile}"
...
RUST_LOG=$RUST_LOG \
risedev ci-start ci-3streaming-2serving-3fe
sqllogictest "${host_args[@]}" -d dev './e2e_test_originalù/ddl/**/*.slt' --junit "parallel-batch-ddl-${profile}"
sqllogictest "${host_args[@]}" -d dev './e2e_test_originalù/visibility_mode/*.slt' -j 16 --junit "parallel-batch-${profile}"
...
RUST_LOG=$RUST_LOG \
risedev ci-start ci-3streaming-2serving-3fe
sqllogictest "${host_args[@]}" -d dev './e2e_test_originalù/generated/**/*.slt' -j 16 --junit "parallel-generated-${profile}"
